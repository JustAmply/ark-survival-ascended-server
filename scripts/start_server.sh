...
ensure_wine_real_binaries() {
  ...
  for name in wine wine64 wine-preloader wine64-preloader; do
    ...
    if ! cat > "$src" << 'EOF'
#!/bin/bash
# Box64 wrapper for Proton Wine binary
# Automatically generated by start_server.sh
SELF="$(readlink -f "$0")"
REAL="${SELF}.real"
if [ ! -x "$REAL" ]; then
...
if ! command -v box64 >/dev/null 2>&1; then
  echo "Error: box64 not found in PATH" >&2
  exit 1
fi
exec box64 "$REAL" "$@"
EOF
    then
      log "Warning: Failed to create wrapper for $name"
      # Attempt to restore the original binary if wrapper creation failed
      if [ -e "$dst" ] && [ ! -e "$src" ]; then
        mv "$dst" "$src" 2>/dev/null || true
      fi
      continue
    fi
    chmod +x "$src" || { log "Warning: Failed to make wrapper executable for $name"; continue; }
    log "Created Box64 wrapper for $(basename "$name")"
  done
}
